...
function HomeScreen({ navigation, route }) {
  ...
  async function startChat() {
    ...
    const { data: profile } = await supabase
      .from('profiles')
      .select('id')
      .eq('username', newEmail.trim())
      .single();
    if (!profile) { Alert.alert('User not found'); return; }
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.row}>
        <TextInput style={[s.input, {flex:1, marginBottom:0}]} placeholder='Username to chat with' value={newEmail} onChangeText={setNewEmail} autoCapitalize='none' />
      ...
      {conversations.length === 0 && <Text style={{color:'#aaa', fontSize:14}}>No conversations yet. Start one above!</Text>}
      {conversations.map((c) => (
